    ...
    func setupUI() {
        ...
        if editingExisting {
            ...
            navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done,
                                                                target: self,
                                                                action: #selector(saveEdit(sender:)))
        } else {
            navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .done,
                                                                target: self,
                                                                action: #selector(performDetailSegue(sender:)))
        }
    }
...
extension InfoViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "infoDetailSegue" {
            if let entity = NSEntityDescription.entity(forEntityName: "InfoSpreadsheet",
                                                       in: DSDataController.shared.viewContext) {
                let infoSpreadsheet = NSManagedObject(entity: entity,
                                                      insertInto: DSDataController.shared.viewContext) as? InfoSpreadsheet ?? InfoSpreadsheet()
